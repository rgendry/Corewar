...
t_arg	*arg_to_byte(t_champ *champ, char *str, char type, t_arg *arg)
{
	t_arg *new;
...
t_instr	*instruction_to_byte(t_champ *champ, char **token, int label)
{
    ...
    {
	    if (i == 1)
            byte_code->arg1 = arg_to_byte(champ, token[1 + label], byte_code->instr, byte_code->arg1);
	    if (i == 2)
            byte_code->arg2 = arg_to_byte(champ, token[2 + label], byte_code->instr, byte_code->arg2);
	    if (i == 3)
            byte_code->arg3 = arg_to_byte(champ, token[3 + label], byte_code->instr, byte_code->arg3);
	    i++;
    }
	byte_code->next = NULL;
